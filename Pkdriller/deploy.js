...
✞Copy the Session id {the one with unreadable codes and long copy it and send it to your deployer or deploy
     𝐇𝐎𝐖 𝐓𝐎 𝐃𝐄𝐏𝐋𝐎𝐘 PKXMD-BOT 
✔ Now check on PKXMD-BOT repository in github fork and give a star to this repository before doing anything 🌟or else PKXMD-BOT owner won't allow you to deploy his bot💀.
...
✔Now click on this devs number and give PKXMD-BOT owner credits https://wa.me/+254785392165
╰────────────────────☆`;
let menuMsg = `
     𝐑𝐞𝐠𝐚𝐫𝐝𝐬 pkdriller🌟`;
   var lien = mybotpic();
   ...
   if (lien.match(/\.(mp4|gif)$/i)) {
    try {
        zk.sendMessage(dest, { video: { url: lien }, caption:infoMsg + menuMsg, footer: "Je suis *pkdriller*, déveloper pk Tech" , gifPlayback : true }, { quoted: ms });
    }
    catch (e) {
...
// Vérification pour .jpeg ou .png
    ...
    try {
        zk.sendMessage(dest, { image: { url: lien }, caption:infoMsg + menuMsg, footer: "Je suis *pkdriller*, déveloper pk Tech" }, { quoted: ms });
    }
    catch (e) {
        console.log("🥵🥵 Menu erreur " + e);
        repondre("🥵🥵 Menu erreur " + e);
